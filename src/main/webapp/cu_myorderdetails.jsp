...
<html>
    ...
    <head>
        ...
        <style>
            ...
            /*//==============product===================*/
            .common .order__items .bill__product:hover{
            ...
            .btn.btn-feedback button{
                color: #FFF;
            }

            .btn.btn-buyNow{
                border: 1.5px #fd7e14 solid;
                border-radius: 3px;
                padding: 0;
            }
            ...
            .btn.btn-buyNow button{
                color: #fd7e14;
                padding: 6px 12px;
            }

            .btn.btn-buyNow button:hover{
                color: #FFF;
                background-color: #fd7e14;
            }
            ...
            <div class="row">
                ...
                <div class="col-md-8 block-right">
                    <div class="d-flex justify-content-between">
                        ...
                        <span>
                            <c:set var="href" value="cus_myorder"/>
                            <c:if test="${not empty type}">
                                <c:set var="href" value="${href}?type=${type}"/>
                            </c:if>
                            <a href="${href}" style="color: #999;">
                                <i class="fa-solid fa-less-than pr-2" style="color: #999"></i>TRỞ LẠI
                            </a>
                        </span>
                        <span>
                            <p class="block-right__title">MÃ ĐƠN HÀNG #${orderId} | 
                                <c:choose>
                                    <c:when test="${order.orStatus eq 'delivered'}"><span class="text-success">HOÀN THÀNH</span></c:when>
                                    <c:when test="${order.orStatus eq 'delivering'}"><span style="color: #ff8306;">ĐANG GIAO</span></c:when>
                                    <c:when test="${order.orStatus eq 'pending'}"><span style="color: #999;">CHỜ XỬ LÝ</span></c:when>
                                    <c:when test="${order.orStatus eq 'cancelled'}"><span class="text-danger">ĐÃ HỦY</span></c:when>
                                </c:choose>
                            </p>
                        </span>
                    </div>
                    <div class="common mb-3">
                        ...
                        <jsp:useBean class="com.team3.onlineshopping.dal.FeedbackDAO" id="feed_dao"/>
                        <jsp:useBean class="com.team3.onlineshopping.dal.CustomerDAO" id="cus_dao"/>
                        <div class="order__bill">
                            <div class="ml-3">
                                ...
                                <div class="text__bill--address">
                                    <div class="bold h6" style="color: rgba(0, 0, 0, .65);">${add_dao.getAddByOrderId(order.orId).addRecName} |
                                        <span style="color: #999;">
                                            ${add_dao.getAddByOrderId(order.orId).addRecPhone}
                                        </span>
                                    </div>
                                    <div style="color: #999;">${add_dao.getAddByOrderId(order.orId).addName}</div>
                                </div>
                            </div>
                            <p class="line"></p>
                            <!--=============================products of bill============================================-->
                            <c:forEach items="${orderdetails}" var="orde">
                                ...
                                <c:set value="${feed_dao.getByProCusOrId(orde.proId, cus_dao.getByAccountId(account.accId).cusId, orde.orId)}" var="feedOrNot"/>
                                ...
                                <div class="bill ">
                                    <div class="bill__product d-flex">
                                        ...
                                        <div class="" style="width: calc(100% - 149px)!important;">
                                            ...
                                            <div style="font-size: 16px;">
                                                <c:if test="${cateSize_dao.getById(orde.cateSizeId).cateSizeName ne 'over'}">
                                    ...
                                    <div class="text-right pb-2">
                                        <c:if test="${order.orStatus eq 'delivered' || order.orStatus eq 'cancelled'}">
                                            <c:if test="${order.orStatus eq 'delivered' && empty feedOrNot}">
                                                <a href="cus_feedback?order=${orde.orId}&proId=${orde.proId}&type=${type}" class="btn btn-feedback mr-2"><button>Đánh Giá</button></a>
                                            </c:if>
                                            <c:if test="${order.orStatus eq 'delivered' && not empty feedOrNot}">
                                                <div style="display: inline-block; padding-right: 1rem;"><p style="color: #fd7e14">Đã đánh giá</p></div>
                                            </c:if>
                                            <a href="pub_productdetails?proId=${orde.proId}" class="btn btn-buyNow mr-3"><button>Mua lại</button></a>
                                        </c:if>
                                    </div>
                                </div>
                            </c:forEach>
                            ...
                            <div class="col-md-12 pb-2">
                                <div class="row me-0 ms-0">
                                    <table>
                                        <tr>
                                            <th style="width: 75%;"><div class="text__bill text-end">Tổng tiền hàng</div></th>
                                            <th style="width: 25%;">
                                        ...
                                        <tr>
                                            <td><div class="text__bill text-end">Giảm giá</div></td>
                                            <td><div class="text__bill text-end">0đ</div></td>
                                        </tr>
                                        <tr>
                                            <td><div class="text__bill text-end">Phí giao hàng</div></td>
                                            <td><div class="text__bill text-end">30,000đ</div></td>
                                        </tr>
                                        <tr>
                                            <td><div class="text__bill text-end">THÀNH TIỀN</div></td>
                                            <td>
                                                <fmt:formatNumber value="${order.orTotalPrice}" pattern="###,###" var="priceBill"/>
                                                <div class="text__bill text-end text-red bold" style="font-size: 18px;">${priceBill}đ</div>
                                            </td>
                                        </tr>
                                    </table>
                                </div>
