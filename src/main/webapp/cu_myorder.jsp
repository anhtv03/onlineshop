        ...
        <div class="container" style="font-family: Arial, Helvetica, sans-serif;">
            ...
            <div class="row">
                ...
                <div class="col-md-8 block-right">
                    <c:choose>
                        <c:when test="${type eq 'all'}"><p class="block-right__title">Đơn hàng của tôi</p></c:when>
                        <c:when test="${type eq 'delivered'}"><p class="block-right__title">Đơn hàng đã giao</p></c:when>
                        <c:when test="${type eq 'delivering'}"><p class="block-right__title">Đơn hàng đang giao</p></c:when>
                        <c:when test="${type eq 'cancelled'}"><p class="block-right__title">Đơn hàng đã hủy</p></c:when>
                        <c:when test="${type eq 'pending'}"><p class="block-right__title">Đơn hàng chờ xử lý</p></c:when>
                    </c:choose>
                    <div class="common mb-3">
                        <jsp:useBean class="com.team3.onlineshopping.dal.ProductDAO" id="pro_dao"/>
                        <jsp:useBean class="com.team3.onlineshopping.dal.OrderDetailsDAO" id="orde_dao"/>
                        <c:forEach items="${orders}" var="orde">
                            <fmt:formatNumber value="${orde.orTotalPrice}" pattern="###,###" var="pricePro"/>
                            <c:set value="${pro_dao.getById(orde_dao.getByOrderId(orde.orId).get(0).getProId())}" var="product"/>
                            <a href="cus_orderdetails?orderId=${orde.orId}&type=${type}" class="d-block order__items">
                                <div class="bill ">
                                    <div class="bill__product d-flex">
                                        ...
                                        <div class="" style="width: calc(100% - 149px)!important;">
                                            <div class=" d-flex align-items-start justify-content-between">
                                                <h5 class="text__bill--product">${orde.orTitle}</h5>
                                            </div>
                                            <div style="font-size: 14px; color: #666666">Ngày đặt: ${orde.orDate}</div>
                                        </div>
                                        <div class="">
                                            <div class="text-left pb-2 d-flex justify-content-center">
                                                <c:choose>
                                                    <c:when test="${orde.orStatus eq 'delivered'}"><span class="text-success">HOÀN THÀNH</span></c:when>
                                                    <c:when test="${orde.orStatus eq 'delivering'}"><span class="text-warning">ĐANG GIAO</span></c:when>
                                                    <c:when test="${orde.orStatus eq 'pending'}"><span style="color: #999;">CHỜ XỬ LÝ</span></c:when>
                                                    <c:when test="${orde.orStatus eq 'cancelled'}"><span class="text-danger">ĐÃ HỦY</span></c:when>
                                                </c:choose>
                                            </div>
                        ...
                        <c:if test="${empty orders}">
                            <div class="row cart__empty">
                                <div class="d-flex justify-content-center">
                                    <img style="width: 100px; height: 100px" src="https://deo.shopeemobile.com/shopee/shopee-pcmall-live-sg/return/5fafbb923393b712b964.png" alt="shop"/>
                                </div>
                                <h4 class="d-flex justify-content-center mt-2">Chưa có đơn hàng</h4>
                            </div>
                        </c:if>
                    </div>
                </div>
        ...
        <div id="toast" class="${status == 'success' ? '' : 'hide'}">
            <div class="toast">
                ...
                <div class="toast__body">
                    <p class="toast__body--msg">Chỉnh sửa thông tin cá nhân thành công</p>
                </div>
            </div>
        </div>
        <!--pop-up notification-->
